...
`Ctrl+B`로 사이드바를 접으면 32px 아이콘 레일만 남긴다. 레일에는 최근항목, 시스템 루트, 데스크탑, 다운로드, 세션 약어 버튼을 표시한다.

세션 약어 버튼은 세션 제목의 첫 글자를 사용하고, 첫 글자가 중복되면 두 글자 약어로 구분한다. 클릭 시 `ContextMenu`를 재사용해 해당 세션 내부 폴더만 표시하며, 접힌 상태에서는 폴더 트리를 직접 펼치지 않는다.
