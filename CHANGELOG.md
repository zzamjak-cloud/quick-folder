...
### Added
- **폴더 우클릭 → AI Agent 요청하기** — 폴더를 우클릭해 AI 에이전트에게 자연어로 작업을 시킬 수 있다. 설정에서 MCP 연동을 등록해 둔 에이전트만 목록에 뜨고, 진행 상황이 앱 안에서 줄 단위로 흐른다. 중단 버튼과 창 닫기 모두 에이전트 프로세스를 정리한다
- 요청은 **우클릭한 폴더 안으로 제한된다** — 작업 폴더를 그 폴더로 두고 파일 도구를 그 안에 가두며, QuickFolder 도구에 넘기는 허용 범위도 그 폴더 하나로 좁힌다. 허용 폴더 밖이면 메뉴 자체가 뜨지 않는다
- 지원 에이전트: **Claude Code · Codex CLI · Gemini CLI**. Claude Desktop·Cursor는 창이 있는 앱이라 외부에서 요청을 넣을 수 없어 제외했다
- AI 에이전트가 쓸 수 있는 기능을 **8개에서 17개로 늘렸다** — 동영상(압축·구간 자르기·이어붙이기·GIF 변환), 노멀맵 등 머티리얼 맵 생성, 스프라이트 시트 묶기·분해, PDF 압축, 이미지 크기 조회, 폴더 용량, 한글(HWP) 본문 추출, 스마트 폴더 병합, 휴지통 복원

### Changed
- 폴더 병합·휴지통 복원도 기존 파괴적 기능과 같게 **확인 없이는 실행하지 않고 계획만 돌려준다**
- **Codex CLI는 폴더 제한을 보장하지 못한다.** 비대화형에서 QuickFolder 도구를 쓰려면 승인 자동화가 필요한데, 그 설정이 폴더 밖 작업 승인까지 함께 열어 버린다. 선택하면 경고와 동의 절차를 거치게 했다

### Fixed
- **에이전트가 동영상 기능을 쓸 때 앱에 이미 들어 있는 FFmpeg를 못 찾던 문제** — 설치 폴더 안에서 앱 본체와 위치가 달라 탐색이 빗나갔다. 번들을 그대로 쓰므로 추가 다운로드가 필요 없다
- **에이전트의 폴더 목록·파일 검색·중복 찾기가 통째로 실패하던 문제** — 응답 형식이 MCP 규격과 달라 AI 클라이언트가 결과를 거부했다
...
### Added
- 픽셀화에 **원본 크기 유지** 옵션 — 이미지 크기는 그대로 두고 픽셀만 정리한다. AI로 생성한 이미지의 흐릿한 블록 경계를 정돈하는 용도
- 픽셀화 미리보기를 **휠로 확대하고 드래그로 옮길 수 있다**. 우측 상단 **맞춤**(전체 보기)·**100%**(1:1) 버튼으로 배율을 잡는다
- 픽셀화 **보기** 버튼 — 같은 미리보기를 전체화면에서 100%로 본다

### Changed
- **픽셀화 설정을 픽셀 크기 하나로 단순화했다** — 출력 크기를 따로 고르던 버튼을 없앴다. 두 값이 서로를 제약해 어느 쪽을 움직여야 원하는 결과가 나오는지 알기 어려웠다. 이제 출력 크기는 픽셀 크기에서 자동으로 정해지고, 결과 규격이 설정 옆에 표시된다
- 픽셀화 팝업을 화면에 맞춰 크게 키우고, **확대 미리보기와 실제 크기 패널을 하나로 합쳤다** — 둘로 나뉘어 각 패널이 좁다 보니 이미지가 잘리고 비율도 어긋나 보였다. 이제 큰 화면 하나에서 확대·축소·이동을 모두 한다
- **미리보기가 저장될 파일과 완전히 같은 이미지가 됐다.** 전에는 속도를 위해 300px로 줄인 뒤 픽셀화해서, 화면에 보이는 색과 저장 결과가 미묘하게 달랐다. 이제 저장과 같은 계산을 그대로 돌린다
- 미리보기가 **100% 배율로 시작한다** — 보이는 크기가 곧 저장될 크기다
- **픽셀화가 2~3배 빨라졌다** — 셀마다 해시맵을 새로 만들던 것, 경계 주사가 캐시를 놓치던 것, 팔레트 초기화가 센터를 매번 다시 훑던 것을 고쳤다. 결과 이미지는 이전과 완전히 동일하다
- 미리보기가 저장과 같은 전체 계산을 돌게 됐지만, 비용이 큰 단계(파일 디코딩·격자 분석)를 팝업이 열려 있는 동안만 임시 보관해 옵션 변경은 여전히 빠르다(2048×1536 기준 컬러 수 변경 48ms). 팝업을 닫으면 즉시 해제한다

### Fixed
- 픽셀화 미리보기가 가로로 찌그러지던 문제 — 이미지가 미리보기 영역보다 크면 가로만 영역 폭에 맞춰 잘려 들어가고 세로는 그대로 남았다
- MCP 수동 등록용 TOML 설정 조각(Codex CLI)이 Windows 경로에서 깨지던 문제 — 백슬래시가 이스케이프로 해석돼(`C:\Users` 의 `\U`) 붙여넣으면 파싱 오류가 났다. 설정 파일에 직접 쓸 때와 같은 경로로 생성하도록 바꿨다
