...
use quickfolder_core::types::{FileEntry, FileType};
...
fn file_mutation_commands_create_write_duplicate_and_detect_conflicts() {
    ...
    quickfolder_core::runtime::block_on(async {
...
fn image_commands_read_dimensions_crop_resize_and_preview() {
    ...
    quickfolder_core::runtime::block_on(async {
        let dimensions = get_image_dimensions(app_paths.clone(), path_string(&image_path))
            .await
...
fn app_handle_commands_list_cache_rename_and_delete_items() {
    ...
    quickfolder_core::runtime::block_on(async {
        let entries = list_directory(app_paths.clone(), path_string(&test_dir.path))
            .await
        ...
        let cached = read_cached_listing(app_paths.clone(), path_string(&test_dir.path))
            .await
        ...
        delete_items(app_paths.clone(), vec![path_string(&renamed_file)], false)
            .await
...
fn file_system_failure_commands_return_errors_without_overwriting() {
    ...
    let read_error = read_text_file(app_paths.clone(), path_string(&missing_file), 1024)
        .expect_err("없는 파일 읽기는 실패해야 함");
    assert!(!read_error.to_string().is_empty());

    quickfolder_core::runtime::block_on(async {
        assert!(
            list_directory(app_paths.clone(), path_string(&missing_file))
                .await
                .is_err(),
            "파일 경로 목록 조회는 실패해야 함"
        );
