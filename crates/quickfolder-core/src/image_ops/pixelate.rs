...
fn apply_pixelate(
    ...
    let cell = pixel_size.clamp(1, w.max(h).max(1));

    let grid = resolve_grid(&src, cell);
    let mut logical = downsample_to_logical(&src, &grid);
    ...
    let target = if output_size > 0 { output_size } else { w.max(h) };
    let factor = (target / logical_max).max(1);
...
fn resolve_grid(img: &image::RgbaImage, cell: u32) -> Grid {
    let (w, h) = (img.width(), img.height());
    let (offset_x, offset_y) = detect_phase(img, cell);
    // 위상만큼 밀린 만큼 열/행이 하나 더 필요할 수 있다.
    let cols = ((w.saturating_sub(offset_x)) as f32 / cell as f32).ceil() as u32;
    let rows = ((h.saturating_sub(offset_y)) as f32 / cell as f32).ceil() as u32;
    Grid {
        cell,
        offset_x,
        offset_y,
...
fn column_edge_strength(img: &image::RgbaImage) -> Vec<f32> {
    ...
    for x in 1..w {
        let mut sum = 0.0f32;
        for y in 0..h {
            sum += pixel_distance(img.get_pixel(x - 1, y), img.get_pixel(x, y));
        }
        out[x as usize] = sum;
    }
    out
...
fn row_edge_strength(img: &image::RgbaImage) -> Vec<f32> {
    ...
    for y in 1..h {
        let mut sum = 0.0f32;
        for x in 0..w {
            sum += pixel_distance(img.get_pixel(x, y - 1), img.get_pixel(x, y));
        }
        out[y as usize] = sum;
    }
    out
...
fn pixel_distance(a: &image::Rgba<u8>, b: &image::Rgba<u8>) -> f32 {
...
fn downsample_to_logical(img: &image::RgbaImage, grid: &Grid) -> image::RgbaImage {
    ...
    for ry in 0..grid.rows {
        let start_y = (grid.offset_y + ry * grid.cell).min(h.saturating_sub(1));
        let end_y = (grid.offset_y + (ry + 1) * grid.cell).min(h).max(start_y + 1);
        for rx in 0..grid.cols {
            let start_x = (grid.offset_x + rx * grid.cell).min(w.saturating_sub(1));
            let end_x = (grid.offset_x + (rx + 1) * grid.cell).min(w).max(start_x + 1);
            let color = cell_representative_color(img, start_x, start_y, end_x, end_y);
            out.put_pixel(rx, ry, color);
        }
    }
    out
}
...
fn cell_representative_color(
    ...
    for y in core_y0..core_y1.min(img.height()) {
        for x in core_x0..core_x1.min(img.width()) {
            ...
            opaque += 1;
            let key = (((p.0[0] >> MODE_BUCKET_SHIFT) as u16) << 10)
                | (((p.0[1] >> MODE_BUCKET_SHIFT) as u16) << 5)
                | ((p.0[2] >> MODE_BUCKET_SHIFT) as u16);
    ...
    // 동률일 때 결과가 흔들리지 않도록 버킷 키를 tie-breaker로 쓴다
    let best = buckets
        .iter()
        .max_by_key(|(key, v)| (v.0, std::cmp::Reverse(**key)));
    match best {
        Some((_, v)) if v.0 > 0 => image::Rgba([
            (v.1 / v.0) as u8,
            (v.2 / v.0) as u8,
            (v.3 / v.0) as u8,
            255,
...
fn upscale_nearest(img: &image::RgbaImage, factor: u32) -> image::RgbaImage {
    ...
    let mut out = image::RgbaImage::new(img.width() * factor, img.height() * factor);
...
fn build_palette(colors: &[([u8; 3], u32)], max_colors: usize) -> Vec<[u8; 3]> {
    ...
    while centers.len() < k {
        ...
        for (color, count) in colors {
            let nearest = centers
                .iter()
                .map(|c| color_distance_sq(*color, *c))
                .min()
                .unwrap_or(0);
            // 점수 = 빈도 x 최근접거리제곱 — 드문 노이즈 색이 팔레트를 차지하지 않게 한다
            let score = nearest as u64 * *count as u64;
            if best.map_or(true, |(_, b)| score > b) {
                best = Some((*color, score));
            }
        }
        match best {
            Some((color, score)) if score > 0 => centers.push(color),
            _ => break,
        }
    }
...
mod tests {
    ...
    fn cell_color_ignores_blurred_border_pixels() {
        ...
        let c = cell_representative_color(&img, 0, 0, 8, 8);
