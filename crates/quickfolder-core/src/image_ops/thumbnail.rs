...
use crate::helpers::*;
use crate::archive_ops::materialize_archive_path_in_cache;
...
pub fn migrate_psd_local_cache_once(app_paths: &AppPaths) {
...
fn thumbnail_cache_keys(path: &str, size: u32) -> Vec<String> {
    ...
    let identity = crate::types::file_identity(&meta);
...
pub(crate) fn invalidate_thumbnail_cache_paths_in_root(app_cache: &Path, paths: &[String]) {
    ...
    for source_path in source_paths {
        ...
        if let Ok(Some(file_id)) =
            crate::system_ops::get_google_drive_file_id_for_path(&source_path)
        {
            for size in THUMBNAIL_CACHE_SIZES {
...
{
    let Some(file_id) = crate::system_ops::get_google_drive_file_id_for_path(path)
        .ok()
        .flatten()
    else {
        return Ok(None);
    ...
    F: FnOnce() -> Result<Option<Vec<u8>>>,
{
    ...
    let identity = if cloud_signature_key {
        ...
        crate::types::file_identity(&meta)
    };
...
fn generate_cloud_image_thumbnail_bytes(
    ...
    if ext == "ico" || ext == "icns" {
        return match generate_image_thumbnail_bytes(path, ext, size) {
            ...
            _ => crate::media_ops::get_os_thumbnail(path, size),
        };
    }
    ...
    crate::media_ops::get_os_thumbnail(path, size)
}
