...
use crate::image_ops::invalidate_thumbnail_cache_paths_in_root;
...
pub async fn copy_items_with_progress(
    ...
    dest: String,
    overwrite: Option<bool>,
    on_progress: crate::progress::Progress<CopyProgress>,
) -> Result<()> {
    let overwrite = overwrite.unwrap_or(false);
...
pub async fn transfer_items_with_progress(
    ...
    dest: String,
    overwrite: Option<bool>,
    on_progress: crate::progress::Progress<TransferQueueProgress>,
) -> Result<()> {
    let overwrite = overwrite.unwrap_or(false);
    let dest_path = std::path::PathBuf::from(dest);
    let app_cache = app_paths.cache_dir().to_path_buf();
