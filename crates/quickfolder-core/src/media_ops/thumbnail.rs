        ...
        for item in chunk {
            ...
            let handle = tokio::task::spawn(async move {
                match task_ft.as_str() {
                    "image" => {
                        crate::image_ops::get_file_thumbnail_path(app_paths, task_path, size)
                            .await
                    }
                    "psd" => {
                        crate::image_ops::get_psd_thumbnail_path(app_paths, task_path, size).await
                    }
                    "video" => get_video_thumbnail_path(app_paths, task_path, size).await,
                    _ => Ok(None),
                }
            });
