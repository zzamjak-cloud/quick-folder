...
- 실시간 검색 및 필터링
- 테마(배경/강조색) 및 UI 줌(확대/축소) 설정 지원
- 카테고리/폴더 제목 텍스트 컬러 프리셋 및 커스텀 컬러 지원
...
- 색상 저장 규칙:
  - `Category.color`: `#RRGGBB`(HEX)로 저장
  - `FolderShortcut.color`: `#RRGGBB`(HEX) 또는 미지정(테마 기본 텍스트 색상)
  - 구형 데이터의 `bg-*` / `text-*` 값은 로딩 시 HEX로 자동 마이그레이션됩니다.
...
- **컬러/테마**:
  - 테마 프리셋 및 커스텀(배경 + 강조색) 지원
  - 카테고리/폴더 제목은 20개 텍스트 컬러 프리셋(흰색 포함) + 커스텀 컬러(피커/HEX 입력) 지원
- **검색 (Search)**:
...
- 카테고리 색상 표시 방식 변경: 원 아이콘 제거 → **제목 텍스트 색상**으로 표시
- 카테고리/폴더 제목 텍스트 색상: **20개 프리셋(흰색 포함) + 커스텀 컬러(피커/HEX)**
- 색상 데이터 마이그레이션: 구형 `bg-*`/`text-*` 값을 HEX로 자동 변환
