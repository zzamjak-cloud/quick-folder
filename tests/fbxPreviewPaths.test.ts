...
import {
  createFbxUrlModifier,
  getFbxTextureFallbackPath,
} from '../components/FileExplorer/fbxPreviewPaths.ts';
...
test('FBX 내부의 오래된 절대 Texture 경로는 현재 FBX 형제 Texture 폴더로 보정한다', () => {
  ...
  assert.equal(
    getFbxTextureFallbackPath(modelPath, staleTextureUrl),
...
test('FBX 텍스처 URL modifier는 보정된 로컬 경로를 asset URL로 바꾼다', () => {
  const modifier = createFbxUrlModifier(modelPath, path => `asset://${path}`);
