...
fn analyze_folder_merge_paths(source: &std::path::Path, dest: &std::path::Path) -> Result<FolderMergeAnalysis> {
...
#[tauri::command]
pub async fn analyze_folder_merge(source: String, dest_parent: String) -> Result<FolderMergeAnalysis> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if !dest_path.is_dir() {
            return Err(AppError::InvalidInput("대상에 같은 이름의 폴더가 없습니다".into()));
        }
        ...
        match mode {
            ...
            FolderMergeConflictMode::Rename => {
                ...
                let parent = dest.parent().ok_or_else(|| {
                    AppError::Internal("대상 파일의 부모 경로 없음".into())
                })?;
