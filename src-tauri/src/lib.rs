...
// 배경 제거 미리보기
#[tauri::command]
async fn remove_white_bg_preview(input: String, threshold: u8, feather: u8, seeds: Vec<[u32; 2]>) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let img = image::open(&input).map_err(|e| format!("이미지 열기 실패: {}", e))?;
    ...
    .await
    .map_err(|e| format!("배경 제거 미리보기 실패: {}", e))?
}
...
async fn remove_white_bg_save(inputs: Vec<String>, threshold: u8, feather: u8, seeds: Vec<[u32; 2]>, trim: bool) -> Result<Vec<String>, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        for input in &inputs {
            let img = image::open(input).map_err(|e| format!("이미지 열기 실패 ({}): {}", input, e))?;
    ...
    .await
    .map_err(|e| format!("배경 제거 저장 실패: {}", e))?
}
...
#[tauri::command]
async fn convert_to_ico(path: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let img = image::open(&path).map_err(|e| format!("이미지 열기 실패: {}", e))?;
        ...
        for &sz in &sizes {
            ...
            icon_dir.add_entry(ico::IconDirEntry::encode(&icon_image)
                .map_err(|e| format!("ICO 인코딩 실패: {}", e))?);
        }
        icon_dir.write(file).map_err(|e| format!("ICO 저장 실패: {}", e))?;
        Ok(out_path.to_string_lossy().into_owned())
    })
    .await
    .map_err(|e| format!("ICO 변환 실패: {}", e))?
}

/// PNG → ICNS 변환 (간단한 ICNS 포맷 — 256px ic08, 512px ic09)
#[tauri::command]
async fn convert_to_icns(path: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let img = image::open(&path).map_err(|e| format!("이미지 열기 실패: {}", e))?;
        let out_path = {
            let p = std::path::Path::new(&path);
        ...
        icon_family.add_icon_with_type(&icns_img_512, icns::IconType::RGBA32_512x512)
            .map_err(|e| format!("ICNS 아이콘 추가 실패: {}", e))?;
        icon_family.write(file).map_err(|e| format!("ICNS 저장 실패: {}", e))?;
        Ok(out_path.to_string_lossy().into_owned())
    })
    .await
    .map_err(|e| format!("ICNS 변환 실패: {}", e))?
}
