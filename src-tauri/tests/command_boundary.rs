use app_lib::{
    check_duplicate_items, compress_to_zip, create_directory, create_text_file, duplicate_items,
    extract_zip, write_text_file,
};
use std::fs;
...
fn file_mutation_commands_create_write_duplicate_and_detect_conflicts() {
    ...
    tauri::async_runtime::block_on(async {
        ...
        let conflicts = check_duplicate_items(vec![path_string(&source_file)], path_string(&destination_dir))
            .await
...
fn archive_commands_compress_and_extract_files() {
    ...
    assert_eq!(
        fs::read_to_string(extract_dir.join("source/root.txt")).expect("압축 해제 루트 파일 읽기 실패"),
        "root"
    );
    assert_eq!(
